<template>
  <!--FOOTER-->
  <footer>
    <div class="navLeftFooter">
      <a href="#" id="navBackToTop">
        <div class="navFooterBackToTop">
          <span class="navFooterBackToTopText">Retour en haut</span>
        </div>
      </a>
      <div class="container-fluid">
        <div class="container">
          <div class="row">
            <div class="col-md-3 col-sm-6">
              <div class="navFooterCol">
                <div class="navFooterColHead">Apprendre a nous connaitre</div>
                <ul>
                  <li>
                    <a href="#">Carrières</a>
                  </li>
                  <li>
                    <a href="#">Blog</a>
                  </li>
                  <li>
                    <a href="#">À propos d'Amazon</a>
                  </li>
                  <li>
                    <a href="#">Relations avec les investisseurs</a>
                  </li>
                  <li>
                    <a href="#">Appareils Amazon</a>
                  </li>
                  <li>
                    <a href="#">Excursions Amazon</a>
                  </li>
                </ul>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="navFooterCol">
                <div class="navFooterColHead">Gagnez de l'argent avec nous</div>
                <ul>
                  <li>
                    <a href="#">Vente sur Amazon</a>
                  </li>
                  <li>
                    <a href="#">Blog</a>
                  </li>
                  <li>
                    <a href="#">À propos d'Amazon</a>
                  </li>
                  <li>
                    <a href="#">Relations avec les investisseurs</a>
                  </li>
                  <li>
                    <a href="#">Appareils Amazon</a>
                  </li>
                  <li>
                    <a href="#">Excursions Amazon</a>
                  </li>
                </ul>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="navFooterCol">
                <div class="navFooterColHead">Produits de paiement Amazon</div>
                <ul>
                  <li>
                    <a href="#">Vente sur Amazon</a>
                  </li>
                  <li>
                    <a href="#">Blog</a>
                  </li>
                  <li>
                    <a href="#">À propos d'Amazon</a>
                  </li>
                  <li>
                    <a href="#">Relations avec les investisseurs</a>
                  </li>
                  <li>
                    <a href="#">Appareils Amazon</a>
                  </li>
                  <li>
                    <a href="#">Excursions Amazon</a>
                  </li>
                </ul>
              </div>
            </div>
            <div class="col-md-3 col-sm-6">
              <div class="navFooterCol">
                <div class="navFooterColHead">Laissez-nous vous aider</div>
                <ul>
                  <li>
                    <a href="#">Vente sur Amazon</a>
                  </li>
                  <li>
                    <a href="#">Blog</a>
                  </li>
                  <li>
                    <a href="#">À propos d'Amazon</a>
                  </li>
                  <li>
                    <a href="#">Relations avec les investisseurs</a>
                  </li>
                  <li>
                    <a href="#">Appareils Amazon</a>
                  </li>
                  <li>
                    <a href="#">Excursions Amazon</a>
                  </li>
                </ul>
              </div>
            </div>
          </div>
        </div>
        <div class="nav-footer-line"></div>
        <div class="container">
          <div class="navFooterPadItem row">
            <div class="col-md-4 col-sm-12 text-right">
              <div class="navFooterLogoLine">
                <a href="#">
                  <div class="nav-logo-base nav-sprite"></div>
                </a>
              </div>
            </div>
            <div class="col-md-8 col-sm-12">
              <span class="icp-container-desktop">
                <div class="navFooterLine">
                  <a href="#" class="icp-button">
                    <div
                      class="icp-nav-globe-img-2 icp-button-globe-2 icp-nav-globe-curr-fix"
                    >
                      ​
                    </div>
                    <span class="icp-color-base">Français</span>
                    <span class="nav-arrow icp-up-down-arrow"></span>
                  </a>
                  <a href="#" class="icp-button">
                    <span class="icp-currency-symbol">EURO</span>
                    <span class="icp-color-base">€</span>
                  </a>
                  <a href="#" class="icp-button">
                    <span class="icp-currency-symbol"></span>
                    <span class="icp-color-base">France</span>
                  </a>
                </div>
              </span>
            </div>
          </div>
        </div>
      </div>
      <div class="navFooterDescLine">
        <div class="container-fluid">
          <div class="container">
            <table>
              <tbody>
                <tr>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Amazon Musique
                      <br />
                      <span class="navFooterDescText">
                        Diffusez des millions <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer" style="width: 4%"></td>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer third" style="width: 4%"></td>
                  <td class="navFooterDescItem third">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer forth" style="width: 4%"></td>
                  <td class="navFooterDescItem forth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer fivth" style="width: 4%"></td>
                  <td class="navFooterDescItem fivth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer sixth" style="width: 4%"></td>
                  <td class="navFooterDescItem sixth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer svnth" style="width: 4%"></td>
                  <td class="navFooterDescItem svnth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
                </tr>
                <tr>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Amazon Musique
                      <br />
                      <span class="navFooterDescText">
                        Diffusez des millions <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer" style="width: 4%"></td>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer third" style="width: 4%"></td>
                  <td class="navFooterDescItem third">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer forth" style="width: 4%"></td>
                  <td class="navFooterDescItem forth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer fivth" style="width: 4%"></td>
                  <td class="navFooterDescItem fivth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer sixth" style="width: 4%"></td>
                  <td class="navFooterDescItem sixth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer svnth" style="width: 4%"></td>
                  <td class="navFooterDescItem svnth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
                </tr>
                <tr>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Amazon Musique
                      <br />
                      <span class="navFooterDescText">
                        Diffusez des millions <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer" style="width: 4%"></td>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer third" style="width: 4%"></td>
                  <td class="navFooterDescItem third">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer forth" style="width: 4%"></td>
                  <td class="navFooterDescItem forth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer fivth" style="width: 4%"></td>
                  <td class="navFooterDescItem fivth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer sixth" style="width: 4%"></td>
                  <td class="navFooterDescItem sixth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer svnth" style="width: 4%"></td>
                  <td class="navFooterDescItem svnth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
                </tr>
                <tr>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Amazon Musique
                      <br />
                      <span class="navFooterDescText">
                        Diffusez des millions <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer" style="width: 4%"></td>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer third" style="width: 4%"></td>
                  <td class="navFooterDescItem third">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer forth" style="width: 4%"></td>
                  <td class="navFooterDescItem forth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer fivth" style="width: 4%"></td>
                  <td class="navFooterDescItem fivth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer sixth" style="width: 4%"></td>
                  <td class="navFooterDescItem sixth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer svnth" style="width: 4%"></td>
                  <td class="navFooterDescItem svnth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                </tr>
                <tr>
                  <td>&nbsp;</td>
                </tr>
                <tr>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Amazon Musique
                      <br />
                      <span class="navFooterDescText">
                        Diffusez des millions <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer" style="width: 4%"></td>
                  <td class="navFooterDescItem">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer third" style="width: 4%"></td>
                  <td class="navFooterDescItem third">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer forth" style="width: 4%"></td>
                  <td class="navFooterDescItem forth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer fivth" style="width: 4%"></td>
                  <td class="navFooterDescItem fivth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer sixth" style="width: 4%"></td>
                  <td class="navFooterDescItem sixth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                  <td class="navFooterDescSpacer svnth" style="width: 4%"></td>
                  <td class="navFooterDescItem svnth">
                    <a href="#" class="nav_a">
                      Publicité Amazon
                      <br />
                      <span class="navFooterDescText">
                        Trouvez, attirez, plus
                        <br />de chansons
                      </span>
                    </a>
                  </td>
                </tr>
              </tbody>
            </table>
          </div>
        </div>
      </div>
      <div class="navFooterCopyright">
        <ul>
          <li>
            <a href="#" class="nav_a">Conditions d'utilisation</a>
          </li>
          <li>
            <a href class="nav_a">Avis de confidentialité</a>
          </li>
          <li>
            <a href class="nav_a">Annonces sur Internet</a>
          </li>
          <li class="nav_last">
            © 1996-2019, Amazon.com, Inc. or its affiliates
          </li>
        </ul>
      </div>
    </div>
  </footer>
  <!--/FOOTER-->
</template>
